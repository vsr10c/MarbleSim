...
#define SENSOR_DEADZONE_MSS     0.06f
...
static int s_calib_feedback_timer = 0;

// Low-pass filtered acceleration
static float s_filt_accel_x = 0.0f;
static float s_filt_accel_y = 0.0f;
...
static void perform_calibration_and_center(qmi8658_dev_t *dev) {
    ...
    // Fast 25-sample average (~50 ms) without freezing the simulation loop
    for (int i = 0; i < 25; i++) {
...
static void screen_touch_event_cb(lv_event_t *e) {
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        // 2. Tapping near the top edge toggles HUD
        if (pt.y < 55) {
        ...
        if (dist_from_marble > 5.0f) {
            float impulse_speed = 350.0f; // px/s
...
static void marble_sim_task(void *arg) {
    ...
    while (1) {
        ...
        // 2. Handle calibration & center request
        ...
        // 3. Calibration feedback timer countdown
        if (s_calib_feedback_timer > 0) {
            s_calib_feedback_timer--;
            if (s_calib_feedback_timer == 0) {
                if (bsp_display_lock(DISPLAY_LOCK_TIMEOUT_MS) == ESP_OK) {
                    marble_render_show_calib_feedback(&s_render_ctx, false);
                    bsp_display_unlock();
                }
            }
        }
        ...
        // 6. Exponential filter to remove micro-tremors while maintaining instant response
        const float alpha = 0.35f;
        s_filt_accel_x = alpha * raw_ax + (1.0f - alpha) * s_filt_accel_x;
        s_filt_accel_y = alpha * raw_ay + (1.0f - alpha) * s_filt_accel_y;

        // 7. Calculate physical tilt angles in degrees
        float tilt_x_deg = asinf(fmaxf(-1.0f, fminf(1.0f, s_filt_accel_x / 9.80665f))) * 57.29578f;
        float tilt_y_deg = asinf(fmaxf(-1.0f, fminf(1.0f, s_filt_accel_y / 9.80665f))) * 57.29578f;

        // 8. Advance physical simulation
        ...
        // 10. Render update under thread-safe LVGL display lock
        if (bsp_display_lock(DISPLAY_LOCK_TIMEOUT_MS) == ESP_OK) {
        ...
        // 11. Precise sleep pacing for 60 FPS
...
void app_main(void) {
    ...
    if (bsp_display_lock(500) == ESP_OK) {
        ...
        // Register touch listener on screen
        lv_obj_add_event_cb(lv_screen_active(), screen_touch_event_cb, LV_EVENT_CLICKED, NULL);
        bsp_display_unlock();
    } else {
        ESP_LOGE(TAG, "Failed to obtain display lock for render initialization");
    }

    // 5. Initialize QMI8658 IMU on shared I2C bus
    ...
    // 6. Set up BOOT button (GPIO 0) for zero calibration
    init_calibration_button();

    // 7. Initial zero-level calibration and marble centering
    if (ret == ESP_OK) {
        perform_calibration_and_center(imu_dev);
    }

    // 8. Launch high-frequency simulation task pinned to CPU Core 1
